...
public class Utils {
		...
		while (true) {
			try {
				...
				if (debug) {
					...
					Utils.logg(log, "info_d", "Click on the xpath:" + msg);
				}
	...
	public static void callSendkeys(WebDriver driver, String Xpath, String Value, int time1, Logger log)
		...
		while (true) {
			try {
				...
				Thread.sleep(1000);
				Utils.logg(log, "ward_d", "Failed to send value  " + Value);
				if (++count == maxTries) {
					Utils.printException(e);
					throw e;
				}
	...
	public static void login(WebDriver driver, String Email, String url, Logger log, String[] csvCell)
		...
		Exception e) {
			Utils.logg(log, "warn_d", "Login to portal failed Quitting the driver" + url);
			printException(e);
			// driver.quit();
			// System.exit(01);
		}
	...
	public static void logout(WebDriver driver, String url, String Role, Logger Log) throws Exception {
		try {
			...
			log.warning("Failure in Logout function");
	...
	public static void checkAcadAndClick(WebDriver driver, String Email, String url, Logger log, String[] csvCell)
		...
		try {
			...
			Utils.logg(log, "info_d", "Failure in checkAcadAndClick function for login id " + Email);
			// checkIfStillInLoginScreenAndLogin(driver, url, Email, log, csvCell);
			try {
				Utils.logg(log, "info_d", "#########################CheckIfStillInLOginScreen function called");

				boolean signInPresent = false;
				signInPresent = driver.findElements(By.xpath("//*[text()='Sign in']")).size() > 0;
				if (signInPresent) {
					Utils.logg(log, "info_d", "Hey Sign In is present, loggin in with id +++++" + Email);
					driver.navigate().to(url);
					...
					Utils.logg(log, "warn", "For this url failed to click on Acad and it's not on Login Page");
				}

			} catch (Exception e1) {
				Utils.printException(e1);
				System.out.println("Failied to login again");
				log.warning("Failed to login again");
				throw (e1);
			}
			// throw (e);
		}
	...
	public static Boolean resouresextra(String url) {
	...
	public static Boolean skipsubject(String url) {
		String urlToMatch = "ecampus|esscisamsung|portal-demo|sbmppsjal|jdinstitutedelhi|ltpct";
		Pattern pt = Pattern.compile(urlToMatch);
		Matcher m = pt.matcher(url);
		while (m.find()) {
			return true;
	...
	@Test
	public static Boolean skipmysudents(String url) {
		String urlToMatch = "dev|demo";
		Pattern pt = Pattern.compile(urlToMatch);
		Matcher m = pt.matcher(url);
		while (m.find()) {
			return true;
	...
	@Test
	public static Boolean QBoption(String url) {
		String urlToMatch = "nsom|ltpct|dev|demo|ecampus|sbmppsjal|bimtech";
		Pattern pt = Pattern.compile(urlToMatch);
		Matcher m = pt.matcher(url);
		while (m.find()) {
			return true;
	...
	public static String[] getClassSubjectAndSection(WebDriver driver, String url, String type, String csvCell[])
		...
		try {
			...
			if (!csvCell[11].equals("NA")) {
				...
				System.out.println("***program string is: " + p1);
				...
				System.out.println("****Subject string is: " + s1);
			} else {
				Utils.clickXpath(driver, ActionXpath.program, time, "click on program", log);
	...
	public static void logg(Logger log, String type, String message) {
		if (type.equals("info_d")) {
			if (debug)
				log.info(message);
